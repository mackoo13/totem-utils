...
for(int i=0; i<RPCount; ++i) {
	h[i] = new TH2D(rpNames0[i].c_str(), rpNames0[i].c_str(), 100, -50, 50, 100, -50, 50);
}
...
while (reader.Next()) {
	for (int i=0; i<ps.GetSize(); i++) {
		...
		if((arm>=0 && arId!=arm) || (stId!=0) || (romanPot>=0 && rpId!=romanPot) || (plane>=0 && plId!=plane)) continue;

		double xVal = ps[i].entryPoint().x();
		double yVal = ps[i].entryPoint().y();
	
		if(mode==1) {
			int rpPosId = 6*arId+rpId;
			double xVal2 = xVal;
			double yVal2 = yVal;
